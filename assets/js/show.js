...
function next_eval()
{
	...
	return (`../../../levels/level${lvl}/level.html`);
...
function show_host(root, h)
{
    ...
    newelem.style.backgroundImage = 'url(../../../assets/img/'+h['img']+')';
...
function show_ifs(root, itf)
{
	...
	newelem.innerHTML = '<table class=if_tab><tr><td colspan=3 class=itf_div_title>interface '+itf['if']+'</td></tr><tr><td>IP</td><td><input size=15 type=text id=ip_'+itf['if']+' value="'+itf['ip']+'" '+ip_active+'><td></tr><tr><td>Mask</td><td><input size=15 type=text id=mask_'+itf['if']+' value="'+itf['mask']+'" '+mask_active+'></td></tr></table>';
	root.appendChild(newelem);
    }
}
...
function all_goals()
{
    ...
    if (nb == goals.length)
    {
		if (g_my_login != '' || !g_eval_lvls) {
			if (level < 10)
				document.getElementById("goals_id").innerHTML += " <input type=button value='Next level' onclick='window.location=next_level(level);'>";
			else
